function [vem]= ArterialBifurcationBoundCond(vem,fluid)
Umax1 = fluid.Umax_1 ;
Umax2 = fluid.Umax_2 ;
NNode = vem.NNode; 
NElemBC = vem.NElemBC  ;
...
if  vem.Node(nodebc1,1) <= 0.01 &&  vem.Node(nodebc2,1) <= 0.01  && ... 
 ...
 %  Nodal Boundary condition Wall| x= 0, 4/6<y<5/6    u = u(y) ,v =0
if  vem.Node(nodebc1,1) <= 0.01 &&  vem.Node(nodebc2,1) <= 0.01  && ... 
    vem.Node(nodebc1,2) >= 4/6  &&  vem.Node(nodebc2,2) >= 4/6  && ...
    vem.Node(nodebc1,2) <= 5/6  &&  vem.Node(nodebc2,2) <= 5/6                
    k = k + 1; 
    kn = kn+1;
    vem.NodeBC(edofbc,1) =1;
    vem.NodeBC(edofbc,2) =1;
    yy = vem.Node(nodebc1,2);
    u = -8*Umax2*(2-3*yy)*(5-6*yy);
    vem.FBC(nodebc1,1) = u;
    yy = vem.Node(nodebc2,2);
    u = -8*Umax2*(2-3*yy)*(5-6*yy);
    vem.FBC(nodebc2,1) = u;
...
if  vem.Node(nodebc1,1) <= 0.209 &&  vem.Node(nodebc2,1) <= 0.209  && ... 
    vem.Node(nodebc1,2) <= 0.167 &&  vem.Node(nodebc2,2) <= 0.167  || ...
    vem.Node(nodebc1,1) <= 0.209 &&  vem.Node(nodebc2,1) <= 0.209  && ... 
    vem.Node(nodebc1,2) >= 0.329 &&  vem.Node(nodebc2,2) >= 0.329  && ...
    vem.Node(nodebc1,2) <= 0.669 &&  vem.Node(nodebc2,2) <= 0.669  || ... 
    vem.Node(nodebc1,1) <= 0.209 &&  vem.Node(nodebc2,1) <= 0.209  && ...
    vem.Node(nodebc1,2) >= 0.829 &&  vem.Node(nodebc2,2) >= 0.829  || ...
...
if  vem.Node(nodebc1,2) <= 0.01 &&  vem.Node(nodebc2,2) <= 0.01
 ...
 %  Nodal Boundary condition Wall| y = 1 ==> u = v = 0 
if  vem.Node(nodebc1,2) >= 0.99 &&  vem.Node(nodebc2,2) >= 0.99
    k = k + 1;
    vem.NodeBC(edofbc,1) =1;
    vem.NodeBC(edofbc,2) =1;
    vem.FBC(edofbc,1) =0.0;
  ...
  %  Nodal Boundary condition Wall| x = 14/10, outlet pressure  
