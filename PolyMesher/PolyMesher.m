...
while(It<=MaxIter && Err>Tol)
  ...
  P = Pc; %Lloyd's update
  x = Domain('FixedPoints',{P}); P=x{1}; % Fixed Points  
  R_P = PolyMshr_Rflct(P,NElem,Domain,Alpha);   %Generate the reflections
  x = Domain('FixedPoints',{P,R_P}); P=x{1}; R_P=x{2}; % Fixed Points
...
title('Domain, Mesh, and Boundary Conditions');
hold off;
%-------------------------------------------------------------------------%
%------------------------ PolyMesher - History ---------------------------%
% version: 1.1 (Aug13)
